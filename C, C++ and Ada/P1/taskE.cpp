...
void Set_AP(int a, int b, int c, int d, depot& mainDepot){
    ...
    for(int k=shelfLength_before; k<d; k++){
        mainDepot.magazyny[a].regaly[b].polki[c].miejsca[k].towar=0;
        mainDepot.magazyny[a].regaly[b].polki[c].miejsca[k].etykieta[0]='\0';
        mainDepot.magazyny[a].regaly[b].polki[c].miejsca[k].etykieta[1]='\0';

    }
    for(int j=d; j<shelfLength_before; j++){
...
void Set_AS(int a, int b, int c, int d, depot& mainDepot){
    ...
    for (int n = rackLength_before; n < c; n++) {
        mainDepot.magazyny[a].regaly[b].polki[n].shelfLength = d;
        for (int j = 0; j < d; j++) {
            mainDepot.magazyny[a].regaly[b].polki[n].miejsca[j].towar = 0;
            mainDepot.magazyny[a].regaly[b].polki[n].miejsca[j].etykieta[0] = '\0';
            mainDepot.magazyny[a].regaly[b].polki[n].miejsca[j].etykieta[1] = '\0';
        }
    }
    for(int n=c; n<rackLength_before; n++ ){
        mainDepot.magazyny[a].regaly[b].polki[n].shelfLength=0;
        for(int j=0; j<128; j++){
            mainDepot.depotStock-=mainDepot.magazyny[a].regaly[b].polki[n].miejsca[j].towar;
            mainDepot.magazyny[a].regaly[b].polki[n].miejsca[j].towar=0;
            mainDepot.magazyny[a].regaly[b].polki[n].miejsca[j].etykieta[0]='\0';
            mainDepot.magazyny[a].regaly[b].polki[n].miejsca[j].etykieta[1]='\0';
        }
...
void Set_AR(int a, int b, int c, int d, depot& mainDepot){
    ...
    for(int n=warehouseLength_before; n<b; n++){
        mainDepot.magazyny[a].regaly[n].rackLength=c;
        for(int j=0; j<c; j++){
            mainDepot.magazyny[a].regaly[n].polki[j].shelfLength=d;
            for(int k=0; k<d; k++){
                mainDepot.magazyny[a].regaly[n].polki[j].miejsca[k].towar=0;
                mainDepot.magazyny[a].regaly[n].polki[j].miejsca[k].etykieta[0]='\0';
                mainDepot.magazyny[a].regaly[n].polki[j].miejsca[k].etykieta[1]='\0';

            }
        }
    }
    for(int n=b; n<warehouseLength_before; n++){
        mainDepot.magazyny[a].regaly[n].rackLength=0;
        for(int j=0; j<128; j++){
            for(int k=0; k<128; k++){
                mainDepot.depotStock-=mainDepot.magazyny[a].regaly[n].polki[j].miejsca[k].towar;
                mainDepot.magazyny[a].regaly[n].polki[j].miejsca[k].towar=0;
                mainDepot.magazyny[a].regaly[n].polki[j].miejsca[k].etykieta[0]='\0';
                mainDepot.magazyny[a].regaly[n].polki[j].miejsca[k].etykieta[1]='\0';
            }
...
void Set_AW(int a, int b, int c, int d, depot& mainDepot){
    ...
    for(int i=a; i<depotLength_before; i++){
        mainDepot.magazyny[i].warehouseLength=0;
        mainDepot.magazyny[i].handyShelf.shelfLength=0;

        for(int j=0; j<128; j++){
            for(int k=0; k<128; k++){
                for(int l=0; l<128; l++){
                    mainDepot.depotStock-=mainDepot.magazyny[i].handyShelf.miejsca[l].towar;
                    mainDepot.magazyny[i].handyShelf.miejsca[l].towar=0;
                    mainDepot.magazyny[i].handyShelf.miejsca[l].etykieta[0]='\0';
                    mainDepot.magazyny[i].handyShelf.miejsca[l].etykieta[1]='\0';
...
void Set_HW(int a, int b, depot& mainDepot){
    ...
    for(int i=b; i<handyShelf_length_before; i++){
...
void Set_HR(int a, int b, depot& mainDepot) {
    ...
    for (int i = a; i < handyRack_length_before; ++i) {
        mainDepot.handyRack.polki[i].shelfLength=0;
        for (int j = 0; j < 128; ++j) {
        ...
        for (int i = 0; i < handyRack_length_before; ++i) {
            int handyRack_shelf_length_before = mainDepot.handyRack.polki[i].shelfLength;
            mainDepot.handyRack.polki[i].shelfLength = b;
            for (int j = handyRack_shelf_length_before; j < b; ++j) {
                mainDepot.handyRack.polki[i].miejsca[j].towar = 0;
                mainDepot.handyRack.polki[i].miejsca[j].etykieta[0] = '\0';
                mainDepot.handyRack.polki[i].miejsca[j].etykieta[1] = '\0';
            }
...
int main() {
    sklad.depotStock = 0;
    sklad.depotLength = 0;
    sklad.depot_handyShelf.shelfLength = 0;
    sklad.handyRack.rackLength = 0;
    ...
    do {
                ...
                for (int i = W; i < 128; i++) {
                    ...
                    for (int j = R; j < 128; j++) {
                        ...
                        for (int k = S; k < 128; k++) {
                            ...
                            sklad.handyRack.polki[k].shelfLength=0;
                            for (int l = P; l < 128; l++) {
                                if (j == R && k == S) {
        ...
        if (porownajNapisy(input, "PUT-W")) {
            ...
            if (w >= sklad.depotLength || r >= sklad.magazyny[w].warehouseLength ||
                ...
                if (A > 65535)A = 65535;
                if (A + sklad.magazyny[w].regaly[r].polki[s].miejsca[p].towar > 65535)
        ...
        if (porownajNapisy(input, "PUT-H")) {
            ...
            if (w >= sklad.depotLength || p >= sklad.magazyny[w].handyShelf.shelfLength) cout << "error" << endl;
                ...
                if(A> 65535)A = 65535;
                if (A+sklad.magazyny[w].handyShelf.miejsca[p].towar>65535)A=65535-sklad.magazyny[w].handyShelf.miejsca[p].towar;
        ...
        if (porownajNapisy(input, "PUT-R")) {
            ...
            if (s >= sklad.handyRack.rackLength || p >= sklad.handyRack.polki[p].shelfLength) cout << "error" << endl;
                ...
                if (A > 65535)A = 65535;
                if (A + sklad.handyRack.polki[s].miejsca[p].towar > 65535)
        ...
        if (porownajNapisy(input, "PUT-S")) {
            ...
            if (p >= sklad.depot_handyShelf.shelfLength) cout << "error" << endl;
                ...
                if (A > 65535)A = 65535;
                if (A + sklad.depot_handyShelf.miejsca[p].towar > 65535)
        ...
        if (porownajNapisy(input, "POP-W")) {
            ...
            if (w >= sklad.depotLength || r >= sklad.magazyny[w].warehouseLength ||
                ...
                if (sklad.magazyny[w].regaly[r].polki[s].miejsca[p].towar < A) A = sklad.magazyny[w].regaly[r].polki[s].miejsca[p].towar;
                sklad.magazyny[w].regaly[r].polki[s].miejsca[p].towar -= A;
                sklad.depotStock -= A;
            }
        }
        ...
        if (porownajNapisy(input, "MOV-W")) {
            ...
            if (wb >= sklad.depotLength || rb >= sklad.magazyny[wb].warehouseLength ||
                ...
                if (A > sklad.magazyny[wb].regaly[rb].polki[sb].miejsca[p].towar)
        ...
        if (porownajNapisy(input, "MOV-R")) {
            ...
            if (w >= sklad.depotLength || r >= sklad.magazyny[w].warehouseLength ||
                ...
                if (A + sklad.handyRack.polki[se].miejsca[p].towar > 65535)A = 65535 - sklad.handyRack.polki[se].miejsca[p].towar;
                if (A > sklad.magazyny[w].regaly[r].polki[sb].miejsca[p].towar)A = sklad.magazyny[w].regaly[r].polki[sb].miejsca[p].towar;
        ...
        if (porownajNapisy(input, "MOV-S")) {
            ...
            if (s >= sklad.handyRack.rackLength || p >= sklad.depot_handyShelf.shelfLength ||
                ...
                if (A + sklad.depot_handyShelf.miejsca[p].towar>65535) A = 65535 - sklad.depot_handyShelf.miejsca[p].towar;
